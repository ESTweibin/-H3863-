...
def main():
    ...
    parser.add_argument('--model-path', type=str, default='models/yolo11n_detect_bayese_640x640_nv12.bin',
                        help="""Path to BPU Quantized *.bin Model.
                                ...
                                RDK S100: Nash-e.
                                RDK S100P: Nash-m.""")
    parser.add_argument('--test-img', type=str, default='../../../resource/assets/bus.jpg',
    ...
    parser.add_argument('--reg', type=int, default=1, help='DFL reg layer.')
    ...
    for class_id, score, bbox in zip(ids, scores, bboxes):
        x1, y1, x2, y2 = bbox
        logger.info("(%d, %d, %d, %d) -> %s: %.2f" % (x1, y1, x2, y2, class_names[class_id], score))
...
class BaseModel:
    ...
    def resizer(self, img: np.ndarray) -> np.ndarray:
        ...
        return cv2.resize(img, (self.model_input_height, self.model_input_weight),
                          interpolation=cv2.INTER_NEAREST)  # 利用resize重新开辟内存
...
class YOLO11_Detect(BaseModel):
    ...
    def postProcess(self, outputs: list[np.ndarray]) -> tuple[list]:
        ...
        l_clses = outputs[5].reshape(-1, 1)   #已经修改


        #classify:单类别检测，直接用分数
        s_scores = s_clses.squeeze()
        s_valid_indices = np.flatnonzero(s_scores >= self.conf_inverse)
        s_ids = np.zeros_like(s_valid_indices)  # 类别ID全为0
        s_scores = s_scores[s_valid_indices]

        m_scores = m_clses.squeeze()
        m_valid_indices = np.flatnonzero(m_scores >= self.conf_inverse)
        m_ids = np.zeros_like(m_valid_indices)
        m_scores = m_scores[m_valid_indices]

        l_scores = l_clses.squeeze()
        l_valid_indices = np.flatnonzero(l_scores >= self.conf_inverse)
        l_ids = np.zeros_like(l_valid_indices)
        l_scores = l_scores[l_valid_indices]
...
class_names = ["Pomacea_canaliculata"]
